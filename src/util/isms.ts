...
/**
 * Escapes characters with special meanings in a Isms passage header (brackets,
 * curly quotes, and backslashes).
 */
export function escapeForIsmsHeader(value: string) {
 ...
 * the start of a line.
 */
export function escapeForIsmsText(value: string) {
	return value.replace(/^::/gm, '\\::');
}

/**
 * Converts a single passage to Isms.
 */
export function passageToIsms(passage: Passage) {
	const escapedName = escapeForIsmsHeader(passage.name)
		.replace(/^\s+/g, match => '\\ '.repeat(match.length))
		.replace(/\s+$/g, match => '\\ '.repeat(match.length));
	const tags =
		passage.tags.length > 0
			? `[${passage.tags.map(escapeForIsmsHeader).join(' ')}]`
			: undefined;
	const metadata = JSON.stringify({
		position: `${passage.left},${passage.top}`,
		size: `${passage.width},${passage.height}`
	}).replace(/\s+/g, '');
	const escapedText = escapeForIsmsText(passage.text);
...
/**
 * Converts Isms source to a passage. If it can't be parsed, then an error is
 * thrown. If it can partially parse the passage, it will do so.
 */
export function passageFromIsms(source: string): Omit<Passage, 'story'> {
	...
	const passage: Omit<Passage, 'story'> = {
		...passageDefaults(),
		id: uuid(),
		name: unescapeForIsmsHeader(rawName.trim()),
		tags: [],
		text: lines.map(unescapeForIsmsText).join('\n').trim()
	};
	...
	if (rawTags) {
		...
		passage.tags = rawTags
			...
			.split(/\s/)
			.filter(tag => tag.trim() !== '')
			.map(tag => unescapeForIsmsHeader(tag));
	}
...
export function storyFromIsms(source: string) {
	...
	if (dataPassageIndex !== -1) {
		...
		console.warn('No StoryData passage is present in Isms');
	}

	// Detect old Isms format, which would have no passage metadata, and put
	// passages in a grid.
...
export function storyToIsms(story: Story) {
	...
	// passages. These passage names are not part of the Isms spec.
...
/**
 * Unescapes characters with special meanings in a Isms passage header (brackets,
 * curly quotes, and backslashes).
 */
export function unescapeForIsmsHeader(value: string) {
 ...
 * the start of a line.
 */
export function unescapeForIsmsText(value: string) {
	return value.replace(/^\\:/gm, ':');
}
